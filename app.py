...
st.markdown("""
    <style>
    /* Ẩn Menu GitHub & Streamlit để bảo mật */
...
clipboard_image = st.chat_input("Dán ảnh vào đây (Ctrl+V) hoặc dùng nút Upload bên dưới...")
...
with st.sidebar:
    ...
    # Lấy danh sách mốc
    cta_docs = db.collection("cta_events").order_by("ts", direction=firestore.Query.DESCENDING).limit(50).stream()
    cta_list = [d.id for d in cta_docs]
    sel_cta = st.selectbox("📌 Chọn mốc làm việc:", cta_list) if cta_list else "Chưa có mốc"

    # --- TÍNH NĂNG MỚI: XÓA HIỂN THỊ MỐC (KHÔNG MẤT DATA ĐIỂM) ---
    if cta_list:
        with st.expander("🗑️ Xóa hiển thị mốc cũ"):
            m_to_hide = st.selectbox("Chọn mốc cần ẩn:", ["-- Chọn mốc --"] + cta_list)
            if m_to_hide != "-- Chọn mốc --":
                if st.button(f"Xóa hiển thị: {m_to_hide}"):
                    db.collection("cta_events").document(m_to_hide).delete()
                    st.warning(f"Đã xóa mốc {m_to_hide} khỏi danh sách hiển thị.")
                    st.rerun()
            ...
            if not api_key:
                ...
                with st.spinner("AI đang đọc danh sách..."):
                    try:
                        ...
                        else: st.error("Lỗi format dữ liệu AI.")
    ...
    if 'temp_data' in st.session_state:
        ...
        if st.button("💾 XÁC NHẬN LƯU VÀ CỘNG ĐIỂM"):
            if sel_cta == "Chưa có mốc": st.error("Bạn chưa chọn mốc!")
                ...
                batch, now = db.batch(), firestore.SERVER_TIMESTAMP
                for i in edited:
...
with t_members:
    ...
    if m_list:
        ...
        st.download_button("📥 Xuất file CSV", data=df.to_csv(index=False).encode('utf-8-sig'), file_name="GE_Report.csv")

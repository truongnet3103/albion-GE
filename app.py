...
st.markdown("""
    ...
    .stTabs [aria-selected="true"] { background-color: #238636 !important; border-radius: 5px; }
...
# --- 2. KẾT NỐI FIREBASE (Sửa lỗi Indentation) ---
@st.cache_resource
def init_firebase():
    if not firebase_admin._apps:
        try:
            ...
            # Xử lý ký tự xuống dòng trong Private Key (Fix lỗi phổ biến trên Cloud)
            if "\\n" in sd["private_key"]:
...
with st.sidebar:
    ...
    st.caption("Quản trị viên: **TruongNET**")
    ...
    new_m = st.text_input("Tên mốc mới (VD: War-01/03):")
    if st.button("✨ Xác nhận Tạo Mốc", use_container_width=True) and new_m:
        db.collection("cta_events").document(new_m).set({
            "name": new_m, 
            "ts": firestore.SERVER_TIMESTAMP
        })
        st.toast(f"Đã tạo mốc {new_m}")
        st.rerun()
    ...
    sel_cta = st.selectbox("📌 Chọn mốc làm việc:", ["Chưa có mốc"] + cta_list)

    st.divider()
    if st.checkbox("⚠️ Chế độ Reset"):
        if st.button("🔥 XÓA SẠCH DATABASE", type="primary", use_container_width=True):
            with st.spinner("Đang dọn dẹp..."):
                for coll in ["members", "cta_attendance", "cta_events"]:
                    docs = db.collection(coll).limit(500).stream()
                    for d in docs: d.reference.delete()
            st.success("Database đã sạch bóng!")
            st.rerun()
...
with t_check:
    ...
    up = st.file_uploader("Kéo thả ảnh Party List (Albion)", type=["jpg", "png", "jpeg"])
    
    if up:
        img = Image.open(up)
        st.image(img, width=500, caption="Ảnh Party")
        
        if st.button("🪄 CHẠY AI PHÂN TÍCH", type="primary"):
            api_key = get_api_key_from_db()
            if not api_key:
                st.error("❌ Không tìm thấy API trên Firebase.")
            elif sel_cta == "Chưa có mốc":
                ...
                with st.spinner("AI đang đọc danh sách..."):
                    try:
                        ...
                        if json_match:
                            ...
                            st.error("AI trả về sai định dạng. Hãy thử lại.")
                    except Exception as e:
                        st.error(f"❌ Lỗi AI: {str(e)}")

    if 'temp_data' in st.session_state:
        st.markdown("#### Xác nhận danh sách")
        edited = st.data_editor(st.session_state['temp_data'], num_rows="dynamic", use_container_width=True)
        
        if st.button("💾 XÁC NHẬN LƯU VÀ CỘNG ĐIỂM", use_container_width=True):
            batch = db.batch()
            now = firestore.SERVER_TIMESTAMP
            for i in edited:
                # Ghi log điểm danh
                doc_id = f"{sel_cta}_{i['name']}".replace(" ", "_")
                batch.set(db.collection("cta_attendance").document(doc_id), {
                    "cta_id": sel_cta, 
                    "name": i['name'], 
                    "role": i['role'], 
                    "ts": now
...
with t_members:
    ...
    for d in docs:
        ...
        m_list.append({
            ...
            "Gia Nhập": m.get("join_date").strftime("%d/%m/%Y") if m.get("join_date") else "N/A",
    ...
    if m_list:
        ...
        st.info("Chưa có thành viên nào trong danh sách.")
...
with t_admin:
    ...
    if target_edit:
        ...
        new_score = st.number_input(f"Số lượt mới cho {target_edit}:", min_value=0, value=curr_score)
        
        col1, col2 = st.columns(2)
        if col1.button("🆙 Cập nhật", use_container_width=True):
            db.collection("members").document(target_edit).update({"count": new_score})
            st.success(f"Đã cập nhật điểm cho {target_edit}")
            st.rerun()
        if col2.button(f"🗑️ Xóa vĩnh viễn {target_edit}", use_container_width=True):
            db.collection("members").document(target_edit).delete()
            st.warning(f"Đã xóa {target_edit}")
            st.rerun()
...
with t_summary:
    ...
    if all_names:
        target_rep = st.selectbox("Xem hồ sơ cá nhân:", all_names)
        if target_rep:
            ...
            with c1:
                st.metric("TỔNG LƯỢT", f"{info.get('count', 0)}")
            ...
            with c2:
                if roles:
                    ...
                    report_text = (
                        ...
                        f"👤 IGN: {target_rep}\n"
                        f"🗓️ Ngày gia nhập: {j_date}\n"
                        f"🔥 Tổng lượt tham gia: {info.get('count', 0)} ({status})\n"
                        f"📊 Role đã chơi: {role_summary}\n"
                        f"--- Quản lý bởi TruongNET ---"
                    )
                    st.text_area("📋 Copy báo cáo nhanh:", value=report_text, height=200)
    else:
